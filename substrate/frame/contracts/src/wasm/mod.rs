...
pub use crate::wasm::runtime::{
	AllowDeprecatedInterface, AllowUnstableInterface, Environment, Memory, ReturnErrorCode,
...
impl<T: Config> WasmBlob<T> {
	...
	fn execute_wasm<E: Ext<T = T>>(
		...
		let runtime = Runtime::new(ext, input_data, Default::default());
	...
	fn execute_riscv<E: Ext<T = T>>(
		...
		let mut state = sp_virtualization::SharedState {
			gas_left: 0,
			exit: false,
			user: Runtime::<_, RiscvMemory<E::T>>::new(ext, input_data, Some(virt.memory())),
		};
