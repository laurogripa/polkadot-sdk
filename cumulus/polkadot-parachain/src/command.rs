...
async fn start_node<Network: sc_network::NetworkBackend<Block, Hash>>(
	...
	match config.chain_spec.runtime() {
		...
		.map(|r| r.0)
		.map_err(Into::into),
		Runtime::CollectivesWestend => crate::service::start_generic_aura_node::<
		...
		.map(|r| r.0)
		.map_err(Into::into),
		Runtime::Shell => crate::service::start_shell_node::<shell_runtime::RuntimeApi, Network>(
			config,
			polkadot_config,
			collator_options,
			id,
		...
		Runtime::Penpal(_) | Runtime::Default =>
			...
			.map(|r| r.0)
			.map_err(Into::into),
		Runtime::GluttonWestend => crate::service::start_basic_lookahead_node::<
